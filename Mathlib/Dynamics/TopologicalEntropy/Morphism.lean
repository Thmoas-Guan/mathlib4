...
lemma dynNet_preimage (h : Semiconj φ S T) {F : Set X} {V : Set (Y × Y)} {n : ℕ} {t : Finset Y}
...
lemma dynCover_preimage (h : Semiconj φ S T) {F : Set X} {V : Set (Y × Y)} (V_symm : SymmetricRel V)
    {n : ℕ} {t : Finset Y} (h' : IsDynCoverOf T (φ '' F) V n t) :
  ...
  exact (dynEntourage_comp_subset T V V n)
    (mem_comp_of_mem_ball (SymmetricRel.dynEntourage T V_symm n) gs_cover.1 hy)
...
theorem coverEntropyInf_of_restrict (u : UniformSpace X) {T : X → X} {F : Set X}
    (h : MapsTo T F F) :
...
theorem coverEntropySup_of_restrict (u : UniformSpace X) {T : X → X} {F : Set X}
    (h : MapsTo T F F) :

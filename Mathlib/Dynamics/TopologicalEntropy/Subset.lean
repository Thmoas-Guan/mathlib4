...
theorem coverEntropyInf_closure [UniformSpace X] {T : X → X} (h : UniformContinuous T) (F : Set X) :
  ...
  exact le_iSup₂_of_le V V_uni ((coverEntropyInfUni_antitone T (closure F) V_U).trans
    (coverEntropyInfUni_closure h F V V_uni))

theorem coverEntropySup_closure [UniformSpace X] {T : X → X} (h : UniformContinuous T) (F : Set X) :
    coverEntropySup T (closure F) = coverEntropySup T F := by
  refine le_antisymm (iSup₂_le fun U U_uni ↦ ?_)
    (coverEntropySup_monotone_space T (subset_closure (s := F)))
  rcases comp_mem_uniformity_sets U_uni with ⟨V, V_uni, V_U⟩
  exact le_iSup₂_of_le V V_uni ((coverEntropySupUni_antitone T (closure F) V_U).trans
...
theorem CoverEntropyInf_finite_iUnion {ι : Type*} [UniformSpace X] [Fintype ι] {T : X → X}
  ...
  exact iSup_congr fun i ↦ (coverEntropyInf_eq_coverEntropySup T (h i)).symm
...
theorem CoverEntropyInf_finite_biUnion {ι : Type*} [UniformSpace X] (s : Finset ι)
  ...
  exact biSup_congr' fun i i_s ↦ (coverEntropyInf_eq_coverEntropySup T (h i i_s)).symm

...
protected theorem IsTopologicalBasisisInducing {β} [TopologicalSpace β] {f : α → β} {T : Set (Set β)}
...
theorem _root_.IsQuotientMap.secondCountableTopology [SecondCountableTopology X] (h' : IsQuotientMap π)
...
theorem IsTopologicalBasis.quotient {V : Set (Set X)} (hV : IsTopologicalBasis V)
  ...
  hV.isQuotientMap quotientMap_quotient_mk' h
...
protected theorem IsInducing.secondCountableTopology [TopologicalSpace β] [SecondCountableTopology β]
    (hf : IsInducing f) : SecondCountableTopology α := by
  rw [hf.1]

...
lemma IsInducing.specializingMap (hf : IsInducing f) (h : StableUnderSpecialization (range f)) :
    SpecializingMap f := by
  intros x y e
  obtain ⟨y, rfl⟩ := h e ⟨x, rfl⟩
  exact ⟨_, hf.specializes_iff.mp e, rfl⟩

lemma IsInducing.generalizingMap (hf : IsInducing f) (h : StableUnderGeneralization (range f)) :
...
theorem quotientMap_mk : IsQuotientMap (mk : X → SeparationQuotient X) :=
  quotientMap_quot_mk
...
theorem inducing_mk : IsInducing (mk : X → SeparationQuotient X) :=
...
theorem quotientMap_prodMap_mk : IsQuotientMap (Prod.map mk mk : X × Y → _) := by
  have hsurj : Surjective (Prod.map mk mk : X × Y → _) := surjective_mk.prodMap surjective_mk
  refine isQuotientMap_iff.2 ⟨hsurj, fun s ↦ ?_⟩

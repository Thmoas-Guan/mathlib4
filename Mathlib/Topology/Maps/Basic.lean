...
lemma isQuotientMap_iff :
    IsQuotientMap f ↔ Surjective f ∧ ∀ s : Set Y, IsOpen s ↔ IsOpen (f ⁻¹' s) :=
  (quotientMap_iff' _).trans <| and_congr Iff.rfl TopologicalSpace.ext_iff

lemma isQuotientMap_iff_closed :
...
protected lemma id : IsQuotientMap (@id X) := ⟨fun x => ⟨x, rfl⟩, coinduced_id.symm⟩

protected lemma comp (hg : IsQuotientMap g) (hf : IsQuotientMap f) : IsQuotientMap (g ∘ f) :=
  ⟨hg.surjective.comp hf.surjective, by rw [hg.eq_coinduced, hf.eq_coinduced, coinduced_compose]⟩

protected lemma of_comp (hf : Continuous f) (hg : Continuous g) (hgf : IsQuotientMap (g ∘ f)) :
    IsQuotientMap g :=
  ⟨hgf.1.of_comp,
    le_antisymm
...
lemma IsOpenEmbedding.map_nhds_eq (hf : IsOpenEmbedding f) (x : X) :
    map f (𝓝 x) = 𝓝 (f x) :=
...
lemma IsOpenEmbedding.tendsto_nhds_iff [TopologicalSpace Z] {f : ι → Y} {l : Filter ι} {y : Y}
    (hg : IsOpenEmbedding g) : Tendsto f l (𝓝 y) ↔ Tendsto (g ∘ f) l (𝓝 (g y)) :=
  hg.isEmbedding.tendsto_nhds_iff

lemma IsOpenEmbedding.tendsto_nhds_iff' (hf : IsOpenEmbedding f) {l : Filter Z} {x : X} :
    Tendsto (g ∘ f) (𝓝 x) l ↔ Tendsto g (𝓝 (f x)) l := by
  rw [Tendsto, ← map_map, hf.map_nhds_eq]; rfl

lemma IsOpenEmbedding.continuousAt_iff [TopologicalSpace Z] (hf : IsOpenEmbedding f) {x : X} :
    ContinuousAt (g ∘ f) x ↔ ContinuousAt g (f x) :=
  hf.tendsto_nhds_iff'

lemma IsOpenEmbedding.continuous (hf : IsOpenEmbedding f) : Continuous f :=
...
alias _root_.IsEmbedding.toIsOpenEmbedding_of_surjective := IsOpenEmbedding.of_isEmbedding
...
lemma IsOpenEmbedding.of_continuous_injective_isOpenMap
...
lemma isOpenEmbedding_iff_continuous_injective_isOpenMap :
  ...
  ⟨fun h => ⟨h.continuous, h.inj, h.isOpenMap⟩, fun h =>
    IsOpenEmbedding.of_continuous_injective_isOpenMap h.1 h.2.1 h.2.2⟩
...
lemma of_isEmpty [IsEmpty X] (f : X → Y) : IsOpenEmbedding f :=
  IsOpenEmbedding.of_isEmbedding_isOpenMap (.of_subsingleton f) (IsOpenMap.of_isEmpty f)

lemma image_mem_nhds {f : X → Y} (hf : IsOpenEmbedding f) {s : Set X} {x : X} :
    f '' s ∈ 𝓝 (f x) ↔ s ∈ 𝓝 x := by
...
lemma of_continuous_injective_isClosedMap (h₁ : Continuous f) (h₂ : Injective f)
  ...
  refine IsClosedEmbedding.of_isEmbedding_isClosedMap ⟨⟨?_⟩, h₂⟩ h₃
...
protected lemma id : IsClosedEmbedding (@id X) := ⟨.id, IsClosedMap.id.isClosed_range⟩

lemma comp (hg : IsClosedEmbedding g) (hf : IsClosedEmbedding f) : IsClosedEmbedding (g ∘ f) :=
...
lemma closure_image_eq (hf : IsClosedEmbedding f) (s : Set X) : closure (f '' s) = f '' closure s :=

...
* `IsDenseInducing i`   means `i` is also `IsInducing`, namely it induces the topology on its codomain;
* `DenseEmbedding e`  means `e` is further an `Embedding`, namely it is injective and `IsInducing`.
...
protected theorem prod [TopologicalSpace γ] [TopologicalSpace δ] {e₁ : α → β} {e₂ : γ → δ}
  ...
  isInducing := de₁.isInducing.prod_map de₂.isInducing
...
theorem mk' (i : α → β) (c : Continuous i) (dense : ∀ x, x ∈ closure (range i))
  ...
  { isInducing := isInducing_iff_nhds.2 fun a =>

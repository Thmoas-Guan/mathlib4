...
theorem Embedding.isLindelof_iff {f : X → Y} (hf : IsEmbedding f) :
...
theorem IsInducing.isLindelof_preimage {f : X → Y} (hf : IsInducing f) (hf' : IsClosed (range f))
...
Moreover, the preimage of a Lindelöf set is Lindelöf, see `IsClosedEmbedding.isLindelof_preimage`. -/

...
lemma Topology.isInducing_const_prod {x : X} {f : Y → Z} :
  ...
  simp_rw [isInducing_iff, instTopologicalSpaceProd, induced_inf, induced_compose, Function.comp,
...
protected lemma IsOpenEmbedding.prod {f : X → Y} {g : Z → W} (hf : IsOpenEmbedding f)
...
theorem IsInducing.codRestrict {e : X → Y} (he : IsInducing e) {s : Set Y} (hs : ∀ x, e x ∈ s) :
    IsInducing (codRestrict e s hs) :=
...
theorem quotientMap_quot_mk : IsQuotientMap (@Quot.mk X r) :=
  ⟨Quot.exists_rep, rfl⟩
...
theorem quotientMap_quotient_mk' : IsQuotientMap (@Quotient.mk' X s) :=
  quotientMap_quot_mk
...
theorem continuous_sigma_map {f₁ : ι → κ} {f₂ : ∀ i, σ i → τ (f₁ i)} :
  ...
  continuous_sigma_iff.trans <| by simp only [Sigma.map, IsEmbedding.sigmaMk.continuous_iff, comp]

...
theorem inducing_comp (g : C(Y, Z)) (hg : IsInducing g) : IsInducing (g.comp : C(X, Y) → C(X, Z)) where
  induced := by
...
theorem embedding_comp (g : C(Y, Z)) (hg : IsEmbedding g) : Embedding (g.comp : C(X, Y) → C(X, Z)) :=

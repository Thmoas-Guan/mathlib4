...
theorem isOpenEmbedding_restrict : IsOpenEmbedding (e.source.restrict e) := by
  refine IsOpenEmbedding.of_continuous_injective_isOpenMap (e.continuousOn.comp_continuous
...
whose source is all of `X`. The converse is also true; see `PartialHomeomorph.to_isOpenEmbedding`. -/
...
noncomputable def partialHomeomorphSubtypeCoe : PartialHomeomorph s X :=
  IsOpenEmbedding.toPartialHomeomorph _ s.2.isOpenEmbedding_subtype_val

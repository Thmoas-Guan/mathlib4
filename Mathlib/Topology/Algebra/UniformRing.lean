...
instance topologicalRing [CommRing α] [TopologicalSpace α] [TopologicalRing α] :
  ...
  toContinuousAdd :=
    IsInducing.continuousAdd (sepQuotRingEquivRingQuot α) (sepQuotHomeomorphRingQuot α)isInducing
  toContinuousMul :=
    IsInducing.continuousMul (sepQuotRingEquivRingQuot α) (sepQuotHomeomorphRingQuot α)isInducing
  toContinuousNeg :=
    IsInducing.continuousNeg (sepQuotHomeomorphRingQuot α)isInducing <|
...
noncomputable def IsDenseInducing.extendRingHom {i : α →+* β} {f : α →+* γ} (ue : IsUniformInducing i)

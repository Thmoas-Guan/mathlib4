...
instance [Inv α] [ContinuousInv α] : ContinuousInv αᵐᵒᵖ :=
  opHomeomorph.symmisInducing.continuousInv unop_inv
...
instance (S : Subgroup G) : TopologicalGroup S :=
  IsInducing.topologicalGroup S.subtype IsInducing.subtypeVal
...
instance topologicalGroup_quotient [N.Normal] : TopologicalGroup (G ⧸ N) where
  continuous_mul := by
    ...
    have quot : IsQuotientMap fun p : G × G ↦ ((p.1 : G ⧸ N), (p.2 : G ⧸ N)) := by

...
theorem IsUniformInducingisInducing {f : α → β} (h : IsUniformInducing f) : IsInducing f := by
...
theorem IsUniformInducing.isDenseInducing {f : α → β} (h : IsUniformInducing f) (hd : DenseRange f) :
    IsDenseInducing f :=
  { dense := hd
    induced := h.isInducing.induced }

theorem SeparationQuotient.isUniformInducing_mk : IsUniformInducing (mk : α → SeparationQuotient α) :=
  ⟨comap_mk_uniformity⟩
...
@[mk_iff]
structure UniformEmbedding (f : α → β) extends IsUniformInducing f : Prop where
...
theorem Filter.HasBasis.uniformEmbedding_iff' {ι ι'} {p : ι → Prop} {p' : ι' → Prop} {s s'}
    ...
    UniformEmbedding f ↔ Injective f ∧
      (∀ i, p' i → ∃ j, p j ∧ ∀ x y, (x, y) ∈ s j → (f x, f y) ∈ s' i) ∧
        (∀ j, p j → ∃ i, p' i ∧ ∀ x y, (f x, f y) ∈ s' i → (x, y) ∈ s j) := by
  rw [uniformEmbedding_iff, and_comm, h.isUniformInducing_iff h']
...
theorem UniformEmbedding.comp {g : β → γ} (hg : UniformEmbedding g) {f : α → β}
  ...
  { hg.toIsUniformInducing.comp hf.toIsUniformInducing with inj := hg.inj.comp hf.inj }

theorem UniformEmbedding.of_comp_iff {g : β → γ} (hg : UniformEmbedding g) {f : α → β} :
    UniformEmbedding (g ∘ f) ↔ UniformEmbedding f := by
  simp_rw [uniformEmbedding_iff, hg.toIsUniformInducing.of_comp_iff, hg.inj.of_comp_iff f]
...
protected theorem IsUniformInducing.uniformEmbedding [T0Space α] {f : α → β}
    (hf : IsUniformInducing f) : UniformEmbedding f :=
  ⟨hf, hfisInducing.injective⟩

theorem uniformEmbedding_iff_isUniformInducing [T0Space α] {f : α → β} :
    UniformEmbedding f ↔ IsUniformInducing f :=
  ⟨UniformEmbedding.toIsUniformInducing, IsUniformInducing.uniformEmbedding⟩
...
theorem uniformEmbedding_of_spaced_out {α} {f : α → β} {s : Set (β × β)} (hs : s ∈ 𝓤 β)
  ...
  exact IsUniformInducing.uniformEmbedding ⟨comap_uniformity_of_spaced_out hs hf⟩

protected theorem UniformEmbedding.embedding {f : α → β} (h : UniformEmbedding f) : IsEmbedding f :=
  { isInducing := h.toIsUniformInducingisInducing
    inj := h.inj }
...
theorem UniformEmbedding.prod {α' : Type*} {β' : Type*} [UniformSpace α'] [UniformSpace β']
  ...
  { h₁.toIsUniformInducing.prod h₂.toIsUniformInducing with inj := h₁.inj.prodMap h₂.inj }
...
theorem isComplete_image_iff {m : α → β} {s : Set α} (hm : IsUniformInducing m) :
  ...
  simp_rw [IsComplete, imp.swap (a := Cauchy _), ← mem_Iic (b := 𝓟 _), fact1.forall fact2,
    hm.cauchy_map_iff, exists_mem_image, map_le_iff_le_comap, hmisInducing.nhds_eq_comap]
...
theorem IsUniformInducing.isComplete_range [CompleteSpace α] {f : α → β} (hf : IsUniformInducing f) :
    IsComplete (range f) :=
...
theorem SeparationQuotient.completeSpace_iff :
  ...
  rw [completeSpace_iff_isComplete_univ, ← range_mk,
    ← completeSpace_iff_isComplete_range isUniformInducing_mk]
...
theorem completeSpace_congr {e : α ≃ β} (he : UniformEmbedding e) :
  ...
  rw [completeSpace_iff_isComplete_range he.toIsUniformInducing, e.range_eq_univ,
    completeSpace_iff_isComplete_univ]

theorem completeSpace_coe_iff_isComplete {s : Set α} : CompleteSpace s ↔ IsComplete s :=
  (completeSpace_iff_isComplete_range uniformEmbedding_subtype_val.toIsUniformInducing).trans <| by
...
instance CompleteSpace.sum [CompleteSpace α] [CompleteSpace β] : CompleteSpace (α ⊕ β) := by
  ...
  exact uniformEmbedding_inl.toIsUniformInducing.isComplete_range.union
    uniformEmbedding_inr.toIsUniformInducing.isComplete_range
...
theorem uniform_extend_subtype [CompleteSpace γ] {p : α → Prop} {e : α → β} {f : α → γ} {b : β}
  ...
  let ⟨c, hc⟩ := uniformly_extend_exists ue'.toIsUniformInducing de'.dense hf ⟨b, this⟩

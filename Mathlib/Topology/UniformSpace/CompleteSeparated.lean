...
theorem UniformEmbedding.toIsClosedEmbedding [UniformSpace α] [UniformSpace β] [CompleteSpace α]
    [T0Space β] {f : α → β} (hf : UniformEmbedding f) :
    IsClosedEmbedding f :=
  ⟨hf.embedding, hf.toIsUniformInducing.isComplete_range.isClosed⟩

...
(see `IsRestrictGen` in `Mathlib/Topology/Defs/Induced.lean`),
and provide the others as corollaries.

## Main results

- `IsRestrictGen.of_seq`: if `X` is a sequential space
  and `S` contains all sets of the form `insert x (Set.range u)`,
  where `u : ℕ → X` is a sequence that converges to `x`,
  then we have `IsRestrictGen S`;

- `IsRestrictGen.isCompact_of_seq`: specialization of the previous lemma
...
namespace IsRestrictGen

protected theorem isOpen_iff (hS : IsRestrictGen S) :
    IsOpen t ↔ ∀ s ∈ S, IsOpen ((↑) ⁻¹' t : Set s) :=
  ⟨fun ht _ _ ↦ ht.preimage continuous_subtype_val, hS.1 t⟩

protected theorem isClosed_iff (hS : IsRestrictGen S) :
...
protected theorem enlarge {T} (hS : IsRestrictGen S) (hT : ∀ s ∈ S, ∃ t ∈ T, s ⊆ t) :
    IsRestrictGen T :=
  of_continuous_prop fun _f hf ↦ hS.continuous_iff.2 fun s hs ↦
    let ⟨t, htT, hst⟩ := hT s hs; (hf t htT).mono hst

protected theorem mono {T} (hS : IsRestrictGen S) (hT : S ⊆ T) : IsRestrictGen T :=
...
lemma isCompact_of_seq [SequentialSpace X] : IsRestrictGen {K : Set X | IsCompact K} :=

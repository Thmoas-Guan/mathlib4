...
variable [StrictOrderedSemiring α] [ExistsAddOfLE α] [OrderedCancelAddCommMonoid β]
  [ExistsAddOfLEOrGE β] [Module α β]
...
lemma smul_add_smul_le_smul_add_smul (ha : a₁ ≤ a₂) (hb : b₁ ≤ b₂) :
  ...
  obtain ⟨b, hb₀, rfl⟩ := exists_nonneg_add_of_le hb
  rw [smul_add, add_right_comm, smul_add, ← add_assoc, add_smul, add_smul]
  gcongr
  exact le_add_of_nonneg_right (smul_nonneg ha₀ hb₀)
...
lemma smul_add_smul_lt_smul_add_smul (ha : a₁ < a₂) (hb : b₁ < b₂) :
  ...
  rw [smul_add, add_right_comm, smul_add, ← add_assoc, add_smul, add_smul]
  gcongr
  exact lt_add_of_pos_right _ (smul_pos ha₀ hb₀)

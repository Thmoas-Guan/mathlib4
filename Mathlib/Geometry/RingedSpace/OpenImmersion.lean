...
def pullbackConeOfLeftFst :
    Y.restrict (TopCat.snd_isOpenEmbedding_of_left_isOpenEmbedding hf.base_open g.base) ⟶ X where
            ...
            (eqToHom
              (by
                ...
                rw [Set.preimage_image_eq _
                    (TopCat.snd_isOpenEmbedding_of_left_isOpenEmbedding hf.base_open g.base).inj]
                rfl))
...
def pullbackConeOfLeft : PullbackCone f g := by
  refine PullbackCone.mk ?_
      (Y.ofRestrict (TopCat.snd_isOpenEmbedding_of_left_isOpenEmbedding H.base_open g.1.base)) ?_
...
is an open immersion iff every stalk map is an iso.
-/
theorem of_stalk_iso {X Y : LocallyRingedSpace} (f : X ⟶ Y) (hf : IsOpenEmbedding f.1.base)
